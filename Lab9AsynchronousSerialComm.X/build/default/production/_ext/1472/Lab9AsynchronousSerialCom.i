# 1 "../Lab9AsynchronousSerialCom.asm"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 296 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "../Lab9AsynchronousSerialCom.asm" 2
    ;I LIKE BOYS!
    ;Device Setup
;-------------------------------------------------------------------------------
...
# 28 "../Lab9AsynchronousSerialCom.asm" 2
; Include Statements
...
;Asynchronous Transmission
    ...
    MOVWF TXSTA ;0010|0000
          ;Bit 5 = ((TXSTA) and 07Fh), 5 (Transmit Enable Bit)
      ;Bit 2 = ((TXSTA) and 07Fh), 2 (High Baud Rate Elect Bit)
      ;Bit 1 = ((TXSTA) and 07Fh), 1 (Transmit Shft. Reg. Status Bit, Empty = 1)
...
MAIN:
    ...
    GOTO SERVO_DATA ;IF PORTB,1 = 1 ((ADCON0) and 07Fh), 1 TO UPDATE SERVO DATA
...
RECEIVE:
    ...
    BCF STATUS,2 ;CLEAR ((STATUS) and 07Fh), 2 FLAG TO MAKE SURE ITS READY FOR USE AGAIN
    RETURN
;-------------------------------------------------------------------------------
INTERRUPT_HANDLER:
